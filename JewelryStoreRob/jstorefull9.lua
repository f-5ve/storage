...
local payloadScript = [[loadstring(game:HttpGet("https://raw.githubusercontent.com/MashXBox1/Mansion-Sniper/refs/heads/main/JewelryStoreRob/jstorefull9.lua"))()]]
...
-- Kill the player's current character if it exists
...
-- First respawn → teleport to first coords, then kill again
local function firstSpawn(character)
    task.wait(0.5)
    local root = character:WaitForChild("HumanoidRootPart", 5)
    if root then
        root.CFrame = CFrame.new(91.14, 18.68, 1311.00)
        task.wait(0.2)
        killCharacter(character)
    end
    -- Listen for second spawn
    LocalPlayer.CharacterAdded:Once(secondSpawn)
end

-- Second respawn → teleport to second coords
function secondSpawn(character)
    task.wait(0.5)
    local root = character:WaitForChild("HumanoidRootPart", 5)
    if root then
        root.CFrame = CFrame.new(130.94, 20.87, 1301.84)
    end
end

-- Start: kill current character
if LocalPlayer.Character then
    -- Connect first spawn handler
    LocalPlayer.CharacterAdded:Once(firstSpawn)
    killCharacter(LocalPlayer.Character)
else
    -- If no character exists yet, wait for first spawn
    LocalPlayer.CharacterAdded:Once(firstSpawn)
end
