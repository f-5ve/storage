...
--[[
    WPF Dark Recode (Compact)
    - Appearance-only refactor; functions and public API unchanged.
    - Palette matches Win11/WPF Dark with subtle elevation.
]]
local Theme = {
    Accent        = Color3.fromRGB(0,120,212),   -- Win blue
    Bg            = Color3.fromRGB(24,24,24),    -- app background
    Surface       = Color3.fromRGB(31,31,31),    -- base panel
    SurfaceAlt    = Color3.fromRGB(45,45,48),    -- subdued panel
    SurfaceHigh   = Color3.fromRGB(63,63,70),    -- elevated/active
    Border        = Color3.fromRGB(68,68,75),    -- strokes
    ...
    Hover         = Color3.fromRGB(72,72,80),
    Pressed       = Color3.fromRGB(86,86,94),
    Disabled      = Color3.fromRGB(90,90,95),
    Shadow        = Color3.fromRGB(0,0,0),
}

-- Density (Compact): smaller paddings, tighter gaps
local Density = {
    Padding = 6,         -- default padding inside containers
    Gap     = 6,         -- default spacing between items
    Radius  = 8,         -- rounded corners
    Stroke  = 1,         -- border thickness
    TextSize = Density.TextSize,       -- base text size
    TitleSize = 16,      -- section titles / buttons
}

-- Helpers that are appearance-only; do not alter external behavior
local function ApplyBasePanel(ui)
    pcall(function()
        ui.BackgroundColor3 = Theme.Surface
        if ui.BorderSizePixel ~= nil then ui.BorderSizePixel = 0 end
        if ui.AutoButtonColor ~= nil then ui.AutoButtonColor = false end
        if ui.TextColor3 ~= nil then ui.TextColor3 = Theme.TextPrimary end
        if ui.TextSize ~= nil then ui.TextSize = Density.TextSize end
    end)
end

local function ApplyElevated(ui, active)
    pcall(function()
        ui.BackgroundColor3 = active and Theme.SurfaceHigh or Theme.SurfaceAlt
        if ui.BorderSizePixel ~= nil then ui.BorderSizePixel = 0 end
    end)
end

local function ApplyAccent(ui)
    pcall(function()
        if ui.ImageColor3 ~= nil then ui.ImageColor3 = Theme.Accent end
        if ui.TextColor3 ~= nil then ui.TextColor3 = Theme.TextPrimary end
        if ui.BackgroundColor3 ~= nil then ui.BackgroundColor3 = Theme.Accent end
    end)
end

local function ApplyText(ui, muted)
    pcall(function()
        ui.TextColor3 = muted and Theme.TextMuted or Theme.TextPrimary
        ui.TextSize = Density.TextSize
    end)
end
...
local function ApplyCorner(parent, radius)
    local ok, inst = pcall(function()
        ...
        c.CornerRadius = UDim.new(0, radius or Density.Radius)
        c.Parent = parent
        return c
    end)
    return ok and inst or nil
...
local function ApplyPadding(parent, pad)
    local ok, inst = pcall(function()
        local p = Instance.new("UIPadding")
        local v = pad or Density.Padding
...
local function ApplyListLayout(parent, gap)
    local ok, inst = pcall(function()
        local l = Instance.new("UIListLayout")
        l.FillDirection = Enum.FillDirection.Vertical
        l.SortOrder = Enum.SortOrder.LayoutOrder
        l.Padding = UDim.new(0, gap or Density.Gap)
        l.Parent = parent
        return l
    end)
    return ok and inst or nil
...
pcall(function() ApplyBasePanel(local Prefabs = Instance.new("Frame")
local Label = Instance.new("TextLabel"):match("local%s+(%w+)")) end)
local Window = Instance.new("ImageLabel")
local Resizer = Instance.new("Frame")
local Bar = Instance.new("Frame")
pcall(function() ApplyBasePanel(local Resizer = Instance.new("Frame")
local Bar = Instance.new("Frame"):match("local%s+(%w+)")) end)
local Toggle = Instance.new("ImageButton")
local Base = Instance.new("ImageLabel")
local Top = Instance.new("ImageLabel")
local Tabs = Instance.new("Frame")
local Title = Instance.new("TextLabel")
pcall(function() ApplyBasePanel(local Tabs = Instance.new("Frame")
local Title = Instance.new("TextLabel"):match("local%s+(%w+)")) end)
local TabSelection = Instance.new("ImageLabel")
local TabButtons = Instance.new("Frame")
local UIListLayout = Instance.new("UIListLayout")
pcall(function() ApplyBasePanel(local TabButtons = Instance.new("Frame")
local UIListLayout = Instance.new("UIListLayout"):match("local%s+(%w+)")) end)
local Frame = Instance.new("Frame")
local Tab = Instance.new("Frame")
pcall(function() ApplyBasePanel(local Frame = Instance.new("Frame")
local Tab = Instance.new("Frame"):match("local%s+(%w+)")) end)
...
pcall(function() ApplyBasePanel(local Objects_2 = Instance.new("Frame")
local UIListLayout_5 = Instance.new("UIListLayout"):match("local%s+(%w+)")) end)
local HorizontalAlignment = Instance.new("Frame")
local UIListLayout_6 = Instance.new("UIListLayout")
pcall(function() ApplyBasePanel(local HorizontalAlignment = Instance.new("Frame")
local UIListLayout_6 = Instance.new("UIListLayout"):match("local%s+(%w+)")) end)
...
pcall(function() ApplyBasePanel(local Indicator_4 = Instance.new("Frame")
local Switch = Instance.new("TextButton"):match("local%s+(%w+)")) end)
...
imgui.Name = "imgui"
pcall(function() ApplyBasePanel(local Windows = Instance.new("Frame")

imgui.Name = "imgui":match("local%s+(%w+)")) end)
imgui.Parent = game:GetService("CoreGui") or gethui()

Prefabs.Name = "Prefabs"
Prefabs.Parent = imgui
Prefabs.BackgroundColor3 = Theme.Surface= UDim2.new(0, 100, 0, 100)
Prefabs.Visible = false


Label.Name = "Label"
Label.Parent = Prefabs
Label.BackgroundColor3 = Theme.Surface= 1
Label.Size = UDim2.new(0, 200, 0, 20)
Label.Font = Enum.Font.Gotham
Label.Text = "Hello, world 123"
Label.TextColor3 = Theme.TextPrimary= Density.TextSize
Label.TextXAlignment = Enum.TextXAlignment.Left

Window.Name = "Window"
Window.Parent = Prefabs
Window.Active = true
Window.BackgroundColor3 = Theme.Surface= 1
Window.ClipsDescendants = true
Window.Position = UDim2.new(0, 20, 0, 20)
Window.Selectable = true
Window.Size = UDim2.new(0, 200, 0, 200)
Window.Image = "rbxassetid://2851926732"
Window.ImageColor3 = Theme.Accent= Enum.ScaleType.Slice
Window.SliceCenter = Rect.new(12, 12, 12, 12)

Resizer.Name = "Resizer"
Resizer.Parent = Window
Resizer.Active = true
Resizer.BackgroundColor3 = Theme.Surface= 1
...
Bar.Name = "Bar"
Bar.Parent = Window
Bar.BackgroundColor3 = Theme.Surface= 0
Bar.Position = UDim2.new(0, 0, 0, 5)
Bar.Size = UDim2.new(1, 0, 0, 15)

Toggle.Name = "Toggle"
Toggle.Parent = Bar
Toggle.BackgroundColor3 = Theme.Surface= 1
...
Base.Name = "Base"
Base.Parent = Bar
Base.BackgroundColor3 = Theme.Surface= 0
Base.Position = UDim2.new(0, 0, 0.800000012, 0)
Base.Size = UDim2.new(1, 0, 0, 10)
Base.Image = "rbxassetid://2851926732"
Base.ImageColor3 = Theme.Accent= Enum.ScaleType.Slice
Base.SliceCenter = Rect.new(12, 12, 12, 12)

Top.Name = "Top"
Top.Parent = Bar
Top.BackgroundColor3 = Theme.Surface= 1
Top.Position = UDim2.new(0, 0, 0, -5)
Top.Size = UDim2.new(1, 0, 0, 10)
Top.Image = "rbxassetid://2851926732"
Top.ImageColor3 = Theme.Accent= Enum.ScaleType.Slice
Top.SliceCenter = Rect.new(12, 12, 12, 12)

Tabs.Name = "Tabs"
Tabs.Parent = Window
Tabs.BackgroundColor3 = Theme.Surface= 1
Tabs.Position = UDim2.new(0, 15, 0, 60)
Tabs.Size = UDim2.new(1, -30, 1, -60)

Title.Name = "Title"
Title.Parent = Window
Title.BackgroundColor3 = Theme.Surface= 1
Title.Position = UDim2.new(0, 30, 0, 3)
Title.Size = UDim2.new(0, 200, 0, 20)
Title.Font = Enum.Font.Gotham
Title.Text = "Gamer Time"
Title.TextColor3 = Theme.TextPrimary= Density.TextSize
Title.TextXAlignment = Enum.TextXAlignment.Left

TabSelection.Name = "TabSelection"
TabSelection.Parent = Window
TabSelection.BackgroundColor3 = Theme.Surface= 1
TabSelection.Position = UDim2.new(0, 15, 0, 30)
TabSelection.Size = UDim2.new(1, -30, 0, 25)
TabSelection.Visible = false
TabSelection.Image = "rbxassetid://2851929490"
TabSelection.ImageColor3 = Theme.Accent= Enum.ScaleType.Slice
TabSelection.SliceCenter = Rect.new(4, 4, 4, 4)

TabButtons.Name = "TabButtons"
TabButtons.Parent = TabSelection
TabButtons.BackgroundColor3 = Theme.Surface= 1
...
Frame.BackgroundColor3 = Theme.Surface= Color3.new(0.105882, 0.164706, 0.207843)
...
Tab.Name = "Tab"
Tab.Parent = Prefabs
Tab.BackgroundColor3 = Theme.Surface= 1
...
TextBox.BackgroundColor3 = Theme.Surface= 1
...
TextBox.Text = ""
TextBox.TextColor3 = Theme.TextPrimary= Density.TextSize

TextBox_Roundify_4px.Name = "TextBox_Roundify_4px"
TextBox_Roundify_4px.Parent = TextBox
TextBox_Roundify_4px.BackgroundColor3 = Theme.Surface= 1
TextBox_Roundify_4px.Size = UDim2.new(1, 0, 1, 0)
TextBox_Roundify_4px.Image = "rbxassetid://2851929490"
TextBox_Roundify_4px.ImageColor3 = Theme.Accent= Enum.ScaleType.Slice
TextBox_Roundify_4px.SliceCenter = Rect.new(4, 4, 4, 4)

Slider.Name = "Slider"
Slider.Parent = Prefabs
Slider.BackgroundColor3 = Theme.Surface= 1
Slider.Position = UDim2.new(0, 0, 0.178571433, 0)
Slider.Size = UDim2.new(1, 0, 0, 20)
Slider.Image = "rbxassetid://2851929490"
Slider.ImageColor3 = Theme.Accent= Enum.ScaleType.Slice
Slider.SliceCenter = Rect.new(4, 4, 4, 4)

Title_2.Name = "Title"
Title_2.Parent = Slider
Title_2.BackgroundColor3 = Theme.Surface= 1
Title_2.Position = UDim2.new(0.5, 0, 0.5, -10)
Title_2.Size = UDim2.new(0, 0, 0, 20)
Title_2.ZIndex = 2
Title_2.Font = Enum.Font.Gotham
Title_2.Text = "Slider"
Title_2.TextColor3 = Theme.TextPrimary= Density.TextSize

Indicator.Name = "Indicator"
Indicator.Parent = Slider
Indicator.BackgroundColor3 = Theme.Surface= 1
Indicator.Size = UDim2.new(0, 0, 0, 20)
Indicator.Image = "rbxassetid://2851929490"
Indicator.ImageColor3 = Theme.Accent= Enum.ScaleType.Slice
Indicator.SliceCenter = Rect.new(4, 4, 4, 4)

Value.Name = "Value"
Value.Parent = Slider
Value.BackgroundColor3 = Theme.Surface= 1
Value.Position = UDim2.new(1, -55, 0.5, -10)
Value.Size = UDim2.new(0, 50, 0, 20)
Value.Font = Enum.Font.Gotham
Value.Text = "0%"
Value.TextColor3 = Theme.TextPrimary= Density.TextSize

TextLabel.Parent = Slider
TextLabel.BackgroundColor3 = Theme.Surface= 1
TextLabel.Position = UDim2.new(1, -20, -0.75, 0)
TextLabel.Size = UDim2.new(0, 26, 0, 50)
TextLabel.Font = Enum.Font.Gotham
TextLabel.Text = "]"
TextLabel.TextColor3 = Theme.TextPrimary= Density.TextSize

TextLabel_2.Parent = Slider
TextLabel_2.BackgroundColor3 = Theme.Surface= 1
TextLabel_2.Position = UDim2.new(1, -65, -0.75, 0)
TextLabel_2.Size = UDim2.new(0, 26, 0, 50)
TextLabel_2.Font = Enum.Font.Gotham
TextLabel_2.Text = "["
TextLabel_2.TextColor3 = Theme.TextPrimary= Density.TextSize

Circle.Name = "Circle"
Circle.Parent = Prefabs
Circle.BackgroundColor3 = Theme.Surface= 1
...
Dropdown.BackgroundColor3 = Theme.Surface= 1
...
Dropdown.TextColor3 = Theme.TextPrimary= Density.TextSize
Dropdown.TextXAlignment = Enum.TextXAlignment.Left

Indicator_2.Name = "Indicator"
Indicator_2.Parent = Dropdown
Indicator_2.BackgroundColor3 = Theme.Surface= 1
...
Box.Name = "Box"
Box.Parent = Dropdown
Box.BackgroundColor3 = Theme.Surface= 1
Box.Position = UDim2.new(0, 0, 0, 25)
Box.Size = UDim2.new(1, 0, 0, 150)
Box.ZIndex = 3
Box.Image = "rbxassetid://2851929490"
Box.ImageColor3 = Theme.Accent= Enum.ScaleType.Slice
Box.SliceCenter = Rect.new(4, 4, 4, 4)

Objects.Name = "Objects"
Objects.Parent = Box
Objects.BackgroundColor3 = Theme.Surface= 1
...
TextButton_Roundify_4px.BackgroundColor3 = Theme.Surface= 1
TextButton_Roundify_4px.Size = UDim2.new(1, 0, 1, 0)
TextButton_Roundify_4px.Image = "rbxassetid://2851929490"
TextButton_Roundify_4px.ImageColor3 = Theme.Accent= Enum.ScaleType.Slice
TextButton_Roundify_4px.SliceCenter = Rect.new(4, 4, 4, 4)

TabButton.Name = "TabButton"
TabButton.Parent = Prefabs
TabButton.BackgroundColor3 = Theme.Surface= 1
...
Objects_2.BackgroundColor3 = Theme.Surface= 1
...
HorizontalAlignment.BackgroundColor3 = Theme.Surface= 1
...
Console.BackgroundColor3 = Theme.Surface= 1
Console.Size = UDim2.new(1, 0, 0, 200)
Console.Image = "rbxassetid://2851928141"
Console.ImageColor3 = Theme.Accent= Enum.ScaleType.Slice
Console.SliceCenter = Rect.new(8, 8, 8, 8)

ScrollingFrame.Parent = Console
ScrollingFrame.BackgroundColor3 = Theme.Surface= 1
...
Source.BackgroundColor3 = Theme.Surface= 1
...
Source.Text = ""
Source.TextColor3 = Theme.TextPrimary= Density.TextSize
...
Saturation.BackgroundColor3 = Theme.Surface= UDim2.new(0.649999976, 0, 0.0500000007, 0)
Saturation.Size = UDim2.new(0, 15, 0, 100)
Saturation.Image = "rbxassetid://3641079629"

Indicator_4.Name = "Indicator"
Indicator_4.Parent = Saturation
Indicator_4.BackgroundColor3 = Theme.Surface= 0
Indicator_4.Size = UDim2.new(0, 20, 0, 2)
Indicator_4.ZIndex = 2

Switch.Name = "Switch"
Switch.Parent = Prefabs
Switch.BackgroundColor3 = Theme.Surface= 1
			...
			function window_data:AddTab(tab_name)
				...
				local function show()
					...
					for i, v in next, tab_buttons:GetChildren() do
						if not (v:IsA("UIListLayout")) then
							v:GetChildren()[1].ImageColor3 = Theme.Accent{Size = UDim2.new(0, v.AbsoluteSize.X, 0, 20)}, options.tween_time)
						end
					end
					...
					new_button:GetChildren()[1].ImageColor3 = Theme.Accent= true
				end
					...
					function tab_data:AddDropdown(dropdown_name, callback)
						...
						function dropdown_data:Add(n)
							...
							object.MouseLeave:Connect(function()
								object.BackgroundColor3 = Theme.Surface= (#objects:GetChildren() - 1) * 20
								if #objects:GetChildren() - 1 >= 10 then
									len = 10 * 20
									objects.CanvasSize = UDim2.new(0, 0, (#objects:GetChildren() - 1) * 0.1, 0)
								end
					...
					function tab_data:AddColorPicker(callback)
						...
						do -- Color Picker Math
							...
							local function update()
								...
								saturation.ImageColor3 = Theme.Accent= Color3.fromHSV(h, s, v)
								sample.ImageColor3 = color
								saturation.ImageColor3 = Theme.Accent= false, false
							...
							function color_picker_data:Set(color)
								...
								sample.ImageColor3 = color
								saturation.ImageColor3 = Theme.Accent:AddConsole(console_options)
						local console_data = {}
...
--[[
    Final pass: apply compact WPF styling to known containers if present.
    This block is safe if the instances don't exist; pcall prevents errors.
]]

pcall(function()
    if script and script.Parent then
        local root = script.Parent
        -- Walk a few common container names without assumptions
        for _, name in ipairs({"Main", "Container", "Content", "Window", "Panel", "Sidebar", "Tabs", "Header", "Footer"}) do
            local obj = root:FindFirstChild(name, true)
            if obj and obj:IsA("GuiObject") then
                ApplyBasePanel(obj)
                ApplyStroke(obj)
                ApplyCorner(obj, Density.Radius)
                ApplyPadding(obj, Density.Padding)
            end
        end
    end
end)
